...
<form name="form1" id="formSearch" method="get" action="">
    搜索:
            标题：<input type="text" style="" name="biaoti" value="<%= request.getParameter("biaoti") !=null ? request.getParameter("biaoti") : "" %>" />            分类：<select data-rule-required="true" data-msg-required="请填写分类" id="fenlei" name="fenlei"   class="class_fenlei2"><option value="">请选择</option><% 
...
<script>
$(".class_fenlei2").val("<%= request.getParameter("fenlei") !=null ? request.getParameter("fenlei") : "" %>")</script>
...
String where = " 1=1 ";
    where += " AND tianjiaren='"+request.getSession().getAttribute("username")+"' ";
...
<td> <%= map.get("dianjishuai") %> </td>
        ...
        <td align="center">
            ...
            <a href="xinwenxinxi_list_tianjiaren.jsp?scid=<%= map.get("id") %>" onClick="return confirm('真的要删除？')">删除</a>
                        <!--qiatnalijne-->
        </td>
    </tr>
