...
<form name="form1" id="formSearch" method="get" action="">
    搜索:
            所属科室：<select id="suoshukeshi" name="suoshukeshi"   class="class_suoshukeshi24"><option value="">请选择</option><% 
...
<script>
$(".class_suoshukeshi24").val("<%= request.getParameter("suoshukeshi") !=null ? request.getParameter("suoshukeshi") : "" %>")</script>
...
String where = " 1=1 ";
         if(request.getParameter("yuyueguahaoid") != null && !"".equals( request.getParameter("yuyueguahaoid") )){     where += " AND yuyueguahaoid='"+request.getParameter("yuyueguahaoid")+"' ";
    }
...
<td> <%= map.get("zhenduanjieguo") %> </td>
<td> <%= map.get("yiliaofeiyong") %> </td>
<td> <%= map.get("kanzhenren") %> </td>
                            <td>
                <%= map.get("iszf") %>                <% if( "否".equals( map.get("iszf") ) ){ %>                <a href="zhifu/?id=<%= map.get("id") %>&biao=jiuzhenjieguo">
                    去支付
                </a>
                <% } %>            </td>
                <td align="center"><%= map.get("addtime") %></td>
        <td align="center">
                                                    <a href="jiuzhenjieguo_updt.jsp?id=<%= map.get("id") %>">修改</a>
            <a href="jiuzhenjieguo_list.jsp?scid=<%= map.get("id") %>" onClick="return confirm('真的要删除？')">删除</a>
                        <!--qiatnalijne-->
        </td>
    </tr>
